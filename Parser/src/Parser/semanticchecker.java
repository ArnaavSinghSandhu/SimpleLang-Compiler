...
class semanticanalysis{
    ...
    public void scopeManagement(ArrayList<TreeNode> root,HashMap<functiontype,HashMap<String,valuetype>> vt,HashMap<String,valuetype> symbolTable){
        ...
        for(Map.Entry<functiontype,HashMap<String,valuetype>> c : vt.entrySet()){
            System.out.println(c.getKey().t);
            for(Map.Entry<String,valuetype> c1 : c.getValue().entrySet()){
                System.out.println(c1.getKey() + c1.getValue().t);
            }
        }
            for(TreeNode i : root){
                Token value = i.data;
                ...
                if(value.t.equals(Token.Type.FUNCTIONAL)){
                    ...
                    if (symbolTable.containsKey(value.value)) continue; // already in table
    ...
    public void functioncheck(TreeNode root,HashMap<functiontype,HashMap<String,valuetype>> vt,functiontype f) throws IllegalArgumentException{
        for(int i = 0 ; i < root.children.size();i++){
            switch(root.children.get(i).data.t){
                ...
                if (i + 1 >= root.children.size()) break;
                TreeNode varNode = root.children.get(i + 1);
                ...
                TreeNode rhs = varNode.children.get(0);
