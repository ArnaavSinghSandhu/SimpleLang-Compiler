...
class parse_statement{
    ...
    while (index < tokens.size()) {
        ...
        switch (tokens.get(index).t) {
            ...
            case FUNCTIONAL:
                TreeNode funcNode = new TreeNode(tokens.get(index));
                root.addChild(funcNode);
                index++;

                if (index < tokens.size() && tokens.get(index).t == Token.Type.IDENTIFIER) {
                    funcNode.addChild(new TreeNode(tokens.get(index)));
                    index++;
                }

                while (index + 1 < tokens.size() &&
                       tokens.get(index).t == Token.Type.ASSIGNMENTKEYWORDS &&
                       tokens.get(index + 1).t == Token.Type.IDENTIFIER) {
                    funcNode.addChild(new TreeNode(tokens.get(index)));
                    funcNode.addChild(new TreeNode(tokens.get(index + 1)));
                    index += 2;
                }

                if (index < tokens.size() && tokens.get(index).t == Token.Type.BODY) {
                    TreeNode bodyNode = new TreeNode(tokens.get(index));
                    funcNode.addChild(bodyNode);
                    index++;

                    while (index < tokens.size() && tokens.get(index).t != Token.Type.END) {
                        Token current = tokens.get(index);

                        if (current.t == Token.Type.ASSIGNMENT) {
                            if (tokens.get(index - 2).t != Token.Type.ASSIGNMENTKEYWORDS) {
                                index++;
                                continue;
                            }
                            TreeNode typeNode = new TreeNode(new Token(Token.Type.ASSIGNMENTKEYWORDS, tokens.get(index - 2).value));
                            TreeNode varNode = new TreeNode(new Token(Token.Type.IDENTIFIER, tokens.get(index - 1).value));
                            TreeNode assignNode = new TreeNode(current);
                            ...
                            while (index < tokens.size() && tokens.get(index).t != Token.Type.END && tokens.get(index).t != Token.Type.RETURN) {
                            ...
                            assignNode.addChild(exprParent);
                            bodyNode.addChild(assignNode);
                            if (index < tokens.size() && tokens.get(index).t == Token.Type.END) index++;
                        ...
                        if (current.t == Token.Type.RETURN) {
                            ...
                            while (index < tokens.size() && tokens.get(index).t != Token.Type.END) {
                                exprTokens.add(tokens.get(index));
                                if(tokens.get(index+1).t == Token.Type.END)break;
                                index++;
                            }
...
class parse_expr_statement{
    ...
    public TreeNode parsefactor() {
        ...
        if (token.t == Token.Type.NUMBER) {
            consume();
            return new TreeNode(token);
        }

        if (token.t == Token.Type.IDENTIFIER) {
             Token idToken = consume();
                TreeNode node = new TreeNode(idToken);
            if (peek() != null && peek().value.equals("(")) {
                ...
                if (peek() != null && peek().value.equals(")")) consume();
                TreeNode callNode = new TreeNode(new Token(Token.Type.FUNCTIONAL, idToken.value));
                for (TreeNode arg : args) callNode.addChild(arg);
                return callNode;
            }
...
class executor{
    ...
    public HashMap<functiontype,HashMap<String,valuetype>> re(TreeNode root,functiontype f){
        symbolTableorg.put(f,new HashMap<>());
        this.functionalvisitor(root.children, symbolTableorg.get(f), operatorTable);
        return symbolTableorg;
    }
    public void functionalvisitor(ArrayList<TreeNode> root,HashMap<String,valuetype> symbolTable,HashMap<String,valuetype> operatorTable){
        ...
        for(TreeNode next : root){
            if (next == null || next.data == null) continue;
            Token value = next.data;
            //System.out.println(value.value);
            if(value.t.equals(Token.Type.FUNCTIONAL)){
                HashMap<String,valuetype> vt1 = new HashMap<>();
                TreeNode lastChild = next.children.isEmpty() ? null : next.children.get(next.children.size() - 1);
                if(lastChild == null || lastChild.data == null || lastChild.data.t != Token.Type.BODY){
                    continue; 
                }
                ...
                functiontype ft1 = new functiontype(funcName, valuetype.Type.INT);
                vt1.put(funcName,v5);
                ...
                if(next.children.size() < 3) throw new RuntimeException("Invalid assignment");

                Token typeToken = next.children.get(1).data;
                Token varToken = next.children.get(0).data;
                TreeNode exprNode = next.children.get(2);

                valuetype.Type vtId = ae.get(typeToken.value);
                symbolTable.put(varToken.value, new valuetype(varToken.value, valuetype.Type.INT));

                functionalvisitor(new ArrayList<>(List.of(exprNode)), symbolTable, operatorTable);
            }
